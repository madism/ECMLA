...
filename = 'BCI_4_DS_1g_calib';
input_filename = 'BCI_4_DS_1g_calib_converted';
output_filename = 'BCI_4_DS_1g_calib_converted.arff';
...
%   1   2  4  8  10  13  16 26 32  42  45  48 49 55 58 59
data = data_to_double(:,[1 2 4 8 10 13 16 26 32 42 45 48 49 55 58 59]);

% For ii length sometimes needs to be changed to 1:m-1;
% Class values also need to be changed (in this case to 1 & -1)
for ii = 1:m-1
    if class_values(ii,1) == 1
          class_1{ii} = data(start_pos(ii,1):start_pos(ii+1,1),:);
    elseif class_values(ii,1) == -1
...
% 1333 rows (4000/3=1333 ms) and 16 columns (16 prechosen EEG channels)
[~,n] = size(class_1_no_empty_cells);
for ii = 1:n
    trials_class_1{1,ii} = class_1_no_empty_cells{1,ii}(1:1333,:);
    trials_class_1{2,ii} = class_1_no_empty_cells{1,ii}(1334:2666,:); 
    trials_class_1{3,ii} = class_1_no_empty_cells{1,ii}(2667:3999,:); 
end
...
for ii = 1:n
    trials_class_2{1,ii} = class_2_no_empty_cells{1,ii}(1:1333,:);
    trials_class_2{2,ii} = class_2_no_empty_cells{1,ii}(1334:2666,:); 
    trials_class_2{3,ii} = class_2_no_empty_cells{1,ii}(2667:3999,:); 
end
